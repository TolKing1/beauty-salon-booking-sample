...
const stringToByteArray$1 = function (e) {
    ...
    encodeByteArray(e, t) {
        ...
        for (let t = 0; t < e.length; t += 3) {
            const o = e[t], s = t + 1 < e.length, i = s ? e[t + 1] : 0, a = t + 2 < e.length, l = a ? e[t + 2] : 0,
                c = o >> 2, u = (3 & o) << 4 | i >> 4;
            let h = (15 & i) << 2 | l >> 6, d = 63 & l;
    ...
    decodeString(e, t) {
        return this.HAS_NATIVE_SUPPORT && !t ? atob(e) : function (e) {
            ...
            for (; r < e.length;) {
                const o = e[r++];
                if (o < 128) t[n++] = String.fromCharCode(o); else if (o > 191 && o < 224) {
                    const s = e[r++];
                    t[n++] = String.fromCharCode((31 & o) << 6 | 63 & s)
                } else if (o > 239 && o < 365) {
                    const s = ((7 & o) << 18 | (63 & e[r++]) << 12 | (63 & e[r++]) << 6 | 63 & e[r++]) - 65536;
                    t[n++] = String.fromCharCode(55296 + (s >> 10)), t[n++] = String.fromCharCode(56320 + (1023 & s))
                } else {
                    const s = e[r++], i = e[r++];
                    t[n++] = String.fromCharCode((15 & o) << 12 | (63 & s) << 6 | 63 & i)
                }
            }
            return t.join("")
    ...
    decodeStringToByteArray(e, t) {
        ...
        for (let t = 0; t < e.length;) {
            const o = r[e.charAt(t++)], s = t < e.length ? r[e.charAt(t)] : 0;
            ++t;
            const i = t < e.length ? r[e.charAt(t)] : 64;
            ++t;
            const a = t < e.length ? r[e.charAt(t)] : 64;
            if (++t, null == o || null == s || null == i || null == a) throw new DecodeBase64StringError;
            const l = o << 2 | s >> 4;
            if (n.push(l), 64 !== i) {
                const e = s << 4 & 240 | i >> 2;
                if (n.push(e), 64 !== a) {
...
const getDefaultsFromGlobal = () => function getGlobal() {
...
}().__FIREBASE_DEFAULTS__, getDefaultsFromCookie = () => {
    ...
    if (r <= 0 || r + 1 === t.length) throw new Error(`Invalid host ${t} with no separate hostname and port!`);
...
class ErrorFactory {
    ...
    create(e, ...t) {
        const r = t[0] || {}, n = `${this.service}/${e}`, o = this.errors[e], i = o ? function replaceTemplate(e, t) {
            return e.replace(s, ((e, r) => {
                const n = t[r];
                return null != n ? String(n) : `<${r}?>`
            }))
        }(o, r) : "Error", a = `${this.serviceName}: ${i} (${n}).`;
...
class StorageError extends FirebaseError {
    constructor(e, t, r = 0) {
        super(prependCode(e), `Firebase Storage: ${t} (${prependCode(e)})`), this.status_ = r, this.customData = {serverResponse: null}, this._baseMessage = this.message, Object.setPrototypeOf(this, StorageError.prototype)
    }
...
function unknown() {
    return new StorageError(i.UNKNOWN, "An unknown error occurred, please check the error payload for server response.")
}

function retryLimitExceeded() {
    return new StorageError(i.RETRY_LIMIT_EXCEEDED, "Max retry time for operation exceeded, please try again.")
}
...
function cannotSliceBlob() {
    return new StorageError(i.CANNOT_SLICE_BLOB, "Cannot slice blob for upload. Please retry the upload.")
}
...
function invalidRootOperation(e) {
    return new StorageError(i.INVALID_ROOT_OPERATION, "The operation '" + e + "' cannot be performed on a root reference, create a non-root reference using child, such as .child('file.png').")
}

function invalidFormat(e, t) {
    return new StorageError(i.INVALID_FORMAT, "String does not match format '" + e + "': " + t)
}
...
class Location {
    ...
    static makeFromBucketSpec(e, t) {
        ...
        throw function invalidDefaultBucket(e) {
            return new StorageError(i.INVALID_DEFAULT_BUCKET, "Invalid default bucket '" + e + "'.")
        }(e)
    }
    ...
    static makeFromUrl(e, t) {
        ...
        const s = t.replace(/[.]/g, "\\."), a = [{
            regex: o, indices: {bucket: 1, path: 3}, postModify: function gsModify(e) {
                "/" === e.path.charAt(e.path.length - 1) && (e.path_ = e.path_.slice(0, -1))
            }
        }, {
            regex: new RegExp(`^https?://${s}/v[A-Za-z0-9_]+/b/${n}/o(/([^?#]*).*)?$`, "i"),
            indices: {bucket: 1, path: 3},
            postModify: httpModify
        }, {
            regex: new RegExp(`^https?://${"firebasestorage.googleapis.com"===t?"(?:storage.googleapis.com|storage.cloud.google.com)":t}/${n}/([^?#]*)`, "i"),
            indices: {bucket: 1, path: 2},
            postModify: httpModify
        }];
        for (let t = 0; t < a.length; t++) {
            const n = a[t], o = n.regex.exec(e);
            if (o) {
        ...
        if (null == r) throw function invalidUrl(e) {
...
function validateNumber(e, t, r, n) {
    if (n < t) throw invalidArgument(`Invalid value for '${e}'. Expected ${t} or greater.`);
    if (n > r) throw invalidArgument(`Invalid value for '${e}'. Expected ${r} or less.`)
}
...
function isRetryStatusCode(e, t) {
    const r = e >= 500 && e < 600, n = -1 !== [408, 429].indexOf(e), o = -1 !== t.indexOf(e);
    return r || n || o
}
...
class NetworkRequest {
    ...
    start_() {
        const doTheRequest = (e, t) => {
            ...
            const progressListener = e => {
                const t = e.loaded, r = e.lengthComputable ? e.total : -1;
                null !== this.progressCallback_ && this.progressCallback_(t, r)
            };
            null !== this.progressCallback_ && r.addUploadProgressListener(progressListener), r.send(this.url_, this.method_, this.body_, this.headers_).then((() => {
                null !== this.progressCallback_ && r.removeUploadProgressListener(progressListener), this.pendingConnection_ = null;
                const t = r.getErrorCode() === a.NO_ERROR, n = r.getStatus();
                if (!t || isRetryStatusCode(n, this.additionalRetryCodes_) && this.retry) {
            ...
            const r = this.resolve_, n = this.reject_, o = t.connection;
            if (t.wasSuccessCode) try {
                ...
                const e = unknown();
                e.serverResponse = o.getErrorText(), this.errorCallback_ ? n(this.errorCallback_(o, e)) : n(e)
            } else if (t.canceled) {
        ...
        this.canceled_ ? backoffDone(0, new RequestEndStatus(!1, null, !0)) : this.backoffId_ = function start(e, t, r) {
            let n = 1, o = null, s = null, i = !1, a = 0;
            ...
            function responseHandler(e, ...t) {
                ...
                if (e) return clearGlobalTimeout(), void triggerCallback.call(null, e, ...t);
                if (canceled() || i) return clearGlobalTimeout(), void triggerCallback.call(null, e, ...t);
                let r;
                n < 64 && (n *= 2), 1 === a ? (a = 2, r = 0) : r = 1e3 * (n + Math.random()), callWithDelay(r)
            }

            let c = !1;

            function stop(e) {
                c || (c = !0, clearGlobalTimeout(), l || (null !== o ? (e || (a = 2), clearTimeout(o), callWithDelay(0)) : e || (a = 1)))
            }

            return callWithDelay(0), s = setTimeout((() => {
                i = !0, stop(!0)
            }), r), stop
    ...
    cancel(e) {
        this.canceled_ = !0, this.appDelete_ = e || !1, null !== this.backoffId_ && function stop(e) {
            e(!1)
...
function getBlob$1(...e) {
    ...
    throw new StorageError(i.UNSUPPORTED_ENVIRONMENT, "This browser doesn't seem to support creating Blobs")
}

function decodeBase64(e) {
    if ("undefined" == typeof atob) throw function missingPolyFill(e) {
        return new StorageError(i.UNSUPPORTED_ENVIRONMENT, `${e} is missing. Make sure to install the required polyfills. See https://firebase.google.com/docs/web/environments-js-sdk#polyfills for more information.`)
    }("base-64");
    return atob(e)
}

const l = {RAW: "raw", BASE64: "base64", BASE64URL: "base64url", DATA_URL: "data_url"};
...
function dataFromString(e, t) {
    switch (e) {
        ...
        case l.DATA_URL:
            return new StringData(function dataURLBytes_(e) {
                const t = new DataURLParts(e);
                return t.base64 ? base64Bytes_(l.BASE64, t.rest) : function percentEncodedBytes_(e) {
                    let t;
                    try {
            ...
            }(t), function dataURLContentType_(e) {
    ...
    for (let r = 0; r < e.length; r++) {
        ...
        if (n <= 127) t.push(n); else if (n <= 2047) t.push(192 | n >> 6, 128 | 63 & n); else if (55296 == (64512 & n)) {
            if (r < e.length - 1 && 56320 == (64512 & e.charCodeAt(r + 1))) {
                n = 65536 | (1023 & n) << 10 | 1023 & e.charCodeAt(++r), t.push(240 | n >> 18, 128 | n >> 12 & 63, 128 | n >> 6 & 63, 128 | 63 & n)
...
function base64Bytes_(e, t) {
    switch (e) {
        case l.BASE64: {
            const r = -1 !== t.indexOf("-"), n = -1 !== t.indexOf("_");
            if (r || n) {
                throw invalidFormat(e, "Invalid character '" + (r ? "-" : "_") + "' found: is it base64url encoded?")
            }
            break
        }
        case l.BASE64URL: {
            const r = -1 !== t.indexOf("+"), n = -1 !== t.indexOf("/");
            if (r || n) {
                throw invalidFormat(e, "Invalid character '" + (r ? "+" : "/") + "' found: is it base64 encoded?")
            }
...
class DataURLParts {
    constructor(e) {
        ...
        if (null === t) throw invalidFormat(l.DATA_URL, "Must be formatted 'data:[<mediatype>][;base64],<data>");
        const r = t[1] || null;
        null != r && (this.base64 = function endsWith(e, t) {
...
class FbsBlob {
    ...
    static getBlob(...e) {
        ...
        {
            const t = e.map((e => isString(e) ? dataFromString(l.RAW, e).data : e.data_));
            let r = 0;
            t.forEach((e => {
                r += e.byteLength
            }));
...
class Mapping {
    constructor(e, t, r, n) {
        this.server = e, this.local = t || e, this.writable = !!r, this.xform = n || noXform_
    }
}
...
function getMappings() {
    if (c) return c;
    const e = [];
    e.push(new Mapping("bucket")), e.push(new Mapping("generation")), e.push(new Mapping("metageneration")), e.push(new Mapping("name", "fullPath", !0));
    ...
    return r.xform = function xformSize(e, t) {
        return void 0 !== t ? Number(t) : t
    }, e.push(r), e.push(new Mapping("timeCreated")), e.push(new Mapping("updated")), e.push(new Mapping("md5Hash", null, !0)), e.push(new Mapping("cacheControl", null, !0)), e.push(new Mapping("contentDisposition", null, !0)), e.push(new Mapping("contentEncoding", null, !0)), e.push(new Mapping("contentLanguage", null, !0)), e.push(new Mapping("contentType", null, !0)), e.push(new Mapping("metadata", "customMetadata", !0)), c = e, c
}

function fromResource(e, t, r) {
    const n = {type: "file"}, o = r.length;
    for (let e = 0; e < o; e++) {
        const o = r[e];
        n[o.local] = o.xform(n, t[o.server])
    }
    return function addRef(e, t) {
        Object.defineProperty(e, "ref", {
            get: function generateRef() {
                const r = e.bucket, n = e.fullPath, o = new Location(r, n);
...
function fromResponseString(e, t, r) {
    ...
    return function fromBackendResponse(e, t, r) {
        ...
        if (r.prefixes) for (const o of r.prefixes) {
            const r = o.replace(/\/$/, ""), s = e._makeStorageReference(new Location(t, r));
            n.prefixes.push(s)
        }
        if (r.items) for (const o of r.items) {
...
function downloadUrlHandler(e, t) {
    return function handler(r, n) {
        const o = fromResourceString(e, n, t);
        return handlerCheck(null !== o), function downloadUrlFromResourceString(e, t, r, n) {
            ...
            return s.split(",").map((t => {
                const o = e.bucket, s = e.fullPath;
                return makeUrl("/b/" + i(o) + "/o/" + i(s), r, n) + makeQueryString({alt: "media", token: t})
            }))[0]
...
function sharedErrorHandler(e) {
    return function errorHandler(t, r) {
        let n;
        return n = 401 === t.getStatus() ? t.getErrorText().includes("Firebase App Check token is invalid") ? function unauthorizedApp() {
            return new StorageError(i.UNAUTHORIZED_APP, "This app does not have permission to access Firebase Storage on this project.")
        }() : function unauthenticated() {
            return new StorageError(i.UNAUTHENTICATED, "User is not authenticated, please authenticate using Firebase Authentication and try again.")
        }() : 402 === t.getStatus() ? function quotaExceeded(e) {
            return new StorageError(i.QUOTA_EXCEEDED, "Quota for bucket '" + e + "' exceeded, please view quota on https://firebase.google.com/pricing/.")
        }(e.bucket) : 403 === t.getStatus() ? function unauthorized(e) {
            return new StorageError(i.UNAUTHORIZED, "User does not have permission to access '" + e + "'.")
        }(e.path) : r, n.status = t.getStatus(), n.serverResponse = r.serverResponse, n
    }
}
...
function objectErrorHandler(e) {
    ...
    return function errorHandler(r, n) {
        let o = t(r, n);
        return 404 === r.getStatus() && (o = function objectNotFound(e) {
            return new StorageError(i.OBJECT_NOT_FOUND, "Object '" + e + "' does not exist.")
        }(e.path)), o.serverResponse = n.serverResponse, o
    }
}

function getMetadata$2(e, t, r) {
    const n = makeUrl(t.fullServerUrl(), e.host, e._protocol), o = e.maxOperationRetryTime,
...
function list$2(e, t, r, n, o) {
    const s = {};
    t.isRoot ? s.prefix = "" : s.prefix = t.path + "/", r && r.length > 0 && (s.delimiter = r), n && (s.pageToken = n), o && (s.maxResults = o);
    const i = makeUrl(t.bucketOnlyServerUrl(), e.host, e._protocol), a = e.maxOperationRetryTime,
        l = new RequestInfo(i, "GET", function listHandler(e, t) {
            return function handler(r, n) {
...
function getBytes$1(e, t, r) {
    const n = makeUrl(t.fullServerUrl(), e.host, e._protocol) + "?alt=media", o = e.maxOperationRetryTime,
        s = new RequestInfo(n, "GET", ((e, t) => t), o);
    return s.errorHandler = objectErrorHandler(t), void 0 !== r && (s.headers.Range = `bytes=0-${r}`, s.successCodes = [200, 206]), s
}

function metadataForUpload_(e, t, r) {
    const n = Object.assign({}, r);
    return n.fullPath = e.path, n.size = t.size(), n.contentType || (n.contentType = function determineContentType_(e, t) {
        return e && e.contentType || t && t.type() || "application/octet-stream"
    }(null, t)), n
}

function multipartUpload(e, t, r, n, o) {
    const s = t.bucketOnlyServerUrl(), i = {"X-Goog-Upload-Protocol": "multipart"};
    ...
    const l = metadataForUpload_(t, n, o),
        ...
        u = "\r\n--" + a + "--", h = FbsBlob.getBlob(c, n, u);
    if (null === h) throw cannotSliceBlob();
    const d = {name: l.fullPath}, p = makeUrl(s, e.host, e._protocol), _ = e.maxUploadRetryTime,
        f = new RequestInfo(p, "POST", metadataHandler(e, r), _);
    return f.urlParams = d, f.headers = i, f.body = h.uploadData(), f.errorHandler = sharedErrorHandler(t), f
}
...
function continueResumableUpload(e, t, r, n, o, s, a, l) {
    ...
    if (a ? (c.current = a.current, c.total = a.total) : (c.current = 0, c.total = n.size()), n.size() !== c.total) throw function serverFileWrongSize() {
        return new StorageError(i.SERVER_FILE_WRONG_SIZE, "Server recorded incorrect upload file size, please retry the upload.")
    }();
    const u = c.total - c.current;
    let h = u;
    o > 0 && (h = Math.min(h, o));
    const d = c.current, p = d + h;
    let _ = "";
    _ = 0 === h ? "finalize" : u === h ? "upload, finalize" : "upload";
    const f = {"X-Goog-Upload-Command": _, "X-Goog-Upload-Offset": `${c.current}`}, g = n.slice(d, p);
    if (null === g) throw cannotSliceBlob();
    const m = t.maxUploadRetryTime, b = new RequestInfo(r, "POST", (function handler(e, r) {
        const o = checkResumeHeader_(e, ["active", "final"]), i = c.current + h, a = n.size();
        let l;
        return l = "final" === o ? metadataHandler(t, s)(e, r) : null, new ResumableUploadStatus(i, a, "final" === o, l)
    }), m);
    return b.headers = f, b.body = g.uploadData(), b.progressCallback = l || null, b.errorHandler = sharedErrorHandler(e), b
}

const u = {STATE_CHANGED: "state_changed"},
    h = {RUNNING: "running", PAUSED: "paused", SUCCESS: "success", CANCELED: "canceled", ERROR: "error"};
...
class Observer {
    constructor(e, t, r) {
        ...
        }(e) || null != t || null != r) this.next = e, this.error = null != t ? t : void 0, this.complete = null != r ? r : void 0; else {
            const t = e;
...
class XhrConnection {
    constructor() {
        this.sent_ = !1, this.xhr_ = new XMLHttpRequest, this.initXhr(), this.errorCode_ = a.NO_ERROR, this.sendPromise_ = new Promise((e => {
    ...
    send(e, t, r, n) {
        ...
        if (this.sent_ = !0, this.xhr_.open(t, e, !0), void 0 !== n) for (const e in n) n.hasOwnProperty(e) && this.xhr_.setRequestHeader(e, n[e].toString());
...
class UploadTask {
    constructor(e, t, r = null) {
        this._transferred = 0, this._needToFetchStatus = !1, this._needToFetchMetadata = !1, this._observers = [], this._error = void 0, this._uploadUrl = void 0, this._request = void 0, this._chunkMultiplier = 1, this._resolve = void 0, this._reject = void 0, this._ref = e, this._blob = t, this._metadata = r, this._mappings = getMappings(), this._resumable = this._shouldDoResumable(this._blob), this._state = "running", this._errorHandler = e => {
            if (this._request = void 0, this._chunkMultiplier = 1, e._codeEquals(i.CANCELED)) this._needToFetchStatus = !0, this.completeTransitions_(); else {
                const t = this.isExponentialBackoffExpired();
                if (isRetryStatusCode(e.status, [])) {
                    if (!t) return this.sleepTime = Math.max(2 * this.sleepTime, 1e3), this._needToFetchStatus = !0, void this.completeTransitions_();
                    e = retryLimitExceeded()
                }
                this._error = e, this._transition("error")
            }
        }, this._metadataErrorHandler = e => {
            this._request = void 0, e._codeEquals(i.CANCELED) ? this.completeTransitions_() : (this._error = e, this._transition("error"))
    ...
    _start() {
        "running" === this._state && void 0 === this._request && (this._resumable ? void 0 === this._uploadUrl ? this._createResumable() : this._needToFetchStatus ? this._fetchStatus() : this._needToFetchMetadata ? this._fetchMetadata() : this.pendingTimeout = setTimeout((() => {
            this.pendingTimeout = void 0, this._continueUpload()
        }), this.sleepTime) : this._oneShotUpload())
    }

    _resolveToken(e) {
        Promise.all([this._ref.storage._getAuthToken(), this._ref.storage._getAppCheckToken()]).then((([t, r]) => {
            switch (this._state) {
                case"running":
                    e(t, r);
                    break;
    ...
    _createResumable() {
        this._resolveToken(((e, t) => {
            const r = function createResumableUpload(e, t, r, n, o) {
                    const s = t.bucketOnlyServerUrl(), i = metadataForUpload_(t, n, o), a = {name: i.fullPath},
                        l = makeUrl(s, e.host, e._protocol), c = {
                            "X-Goog-Upload-Protocol": "resumable",
                            "X-Goog-Upload-Command": "start",
                            "X-Goog-Upload-Header-Content-Length": `${n.size()}`,
                            "X-Goog-Upload-Header-Content-Type": i.contentType,
                            "Content-Type": "application/json; charset=utf-8"
                        }, u = toResourceString(i, r), h = e.maxUploadRetryTime,
                        d = new RequestInfo(l, "POST", (function handler(e) {
                            let t;
                    ...
                    return d.urlParams = a, d.headers = c, d.body = u, d.errorHandler = sharedErrorHandler(t), d
            ...
            this._request = n, n.getPromise().then((e => {
    ...
    _fetchStatus() {
        ...
        this._resolveToken(((t, r) => {
            const n = function getResumableUploadStatus(e, t, r, n) {
                    const o = e.maxUploadRetryTime, s = new RequestInfo(r, "POST", (function handler(e) {
                        ...
                        r || handlerCheck(!1);
                        const o = Number(r);
                        return handlerCheck(!isNaN(o)), new ResumableUploadStatus(o, n.size(), "final" === t)
                    }), o);
                    return s.headers = {"X-Goog-Upload-Command": "query"}, s.errorHandler = sharedErrorHandler(t), s
    ...
    _continueUpload() {
        const e = 262144 * this._chunkMultiplier, t = new ResumableUploadStatus(this._transferred, this._blob.size()),
            r = this._uploadUrl;
        this._resolveToken(((n, o) => {
            let s;
            try {
                s = continueResumableUpload(this._ref._location, this._ref.storage, r, this._blob, e, this._mappings, t, this._makeProgressCallback())
            } catch (e) {
    ...
    _increaseMultiplier() {
        2 * (262144 * this._chunkMultiplier) < 33554432 && (this._chunkMultiplier *= 2)
    }

    _fetchMetadata() {
        this._resolveToken(((e, t) => {
            const r = getMetadata$2(this._ref.storage, this._ref._location, this._mappings),
                n = this._ref.storage._makeRequest(r, newTextConnection, e, t);
            this._request = n, n.getPromise().then((e => {
    ...
    _oneShotUpload() {
        this._resolveToken(((e, t) => {
            const r = multipartUpload(this._ref.storage, this._ref._location, this._mappings, this._blob, this._metadata),
                n = this._ref.storage._makeRequest(r, newTextConnection, e, t);
            this._request = n, n.getPromise().then((e => {
    ...
    _transition(e) {
        if (this._state !== e) switch (e) {
            case"canceling":
            case"pausing":
                this._state = e, void 0 !== this._request ? this._request.cancel() : this.pendingTimeout && (clearTimeout(this.pendingTimeout), this.pendingTimeout = void 0, this.completeTransitions_());
                break;
            case"running":
            ...
            case"success":
                ...
                break;
            case"canceled":
                this._error = canceled(), this._state = e, this._notifyObservers()
        }
    }
...
class Reference {
    constructor(e, t) {
        this._service = e, this._location = t instanceof Location ? t : Location.makeFromUrl(t, e.host)
    }
...
function uploadBytes$1(e, t, r) {
    ...
    const n = multipartUpload(e.storage, e._location, getMappings(), new FbsBlob(t, !0), r);
    return e.storage.makeRequestWithTokens(n, newTextConnection).then((t => ({metadata: t, ref: e})))
}
...
async function listAllHelper(e, t, r) {
    const n = {pageToken: r}, o = await list$1(e, n);
    t.prefixes.push(...o.prefixes), t.items.push(...o.items), null != o.nextPageToken && await listAllHelper(e, t, o.nextPageToken)
}

function list$1(e, t) {
    null != t && "number" == typeof t.maxResults && validateNumber("options.maxResults", 1, 1e3, t.maxResults);
    const r = t || {}, n = list$2(e.storage, e._location, "/", r.pageToken, r.maxResults);
...
function updateMetadata$1(e, t) {
    ...
    const r = function updateMetadata$2(e, t, r, n) {
        const o = makeUrl(t.fullServerUrl(), e.host, e._protocol), s = toResourceString(r, n),
            i = e.maxOperationRetryTime, a = new RequestInfo(o, "PATCH", metadataHandler(e, n), i);
        return a.headers = {"Content-Type": "application/json; charset=utf-8"}, a.body = s, a.errorHandler = objectErrorHandler(t), a
...
function getDownloadURL$1(e) {
    ...
    return e.storage.makeRequestWithTokens(t, newTextConnection).then((e => {
        if (null === e) throw function noDownloadURL() {
            return new StorageError(i.NO_DOWNLOAD_URL, "The given file does not have any download URLs.")
        }();
        return e
    }))
}
...
function deleteObject$1(e) {
    ...
    const t = function deleteObject$2(e, t) {
        const r = makeUrl(t.fullServerUrl(), e.host, e._protocol), n = e.maxOperationRetryTime,
            o = new RequestInfo(r, "DELETE", (function handler(e, t) {
            }), n);
        return o.successCodes = [200, 204], o.errorHandler = objectErrorHandler(t), o
...
function _getChild$1(e, t) {
    const r = function child(e, t) {
        const r = t.split("/").filter((e => e.length > 0)).join("/");
        return 0 === e.length ? r : e + "/" + r
    }(e._location.path, t), n = new Location(e._location.bucket, r);
...
function refFromPath(e, t) {
    if (e instanceof FirebaseStorageImpl) {
        const r = e;
        if (null == r._bucket) throw function noDefaultBucket() {
            return new StorageError(i.NO_DEFAULT_BUCKET, "No default bucket found. Did you set the 'storageBucket' property when initializing the app?")
        }();
...
function ref$1(e, t) {
    if (t && function isUrl(e) {
        return /^[A-Za-z]+:\/\//.test(e)
    }(t)) {
        if (e instanceof FirebaseStorageImpl) return function refFromURL(e, t) {
            return new Reference(e, t)
        }(e, t);
...
function connectStorageEmulator$1(e, t, r, n = {}) {
    ...
    o && (e._overrideAuthToken = "string" == typeof o ? o : function createMockUserToken(e, t) {
        ...
        const r = t || "demo-project", n = e.iat || 0, o = e.sub || e.user_id;
...
class FirebaseStorageImpl {
    ...
    set host(e) {
        this._host = e, null != this._url ? this._bucket = Location.makeFromBucketSpec(this._url, e) : this._bucket = extractBucket(e, this.app.options)
    }
    ...
    set maxUploadRetryTime(e) {
        validateNumber("time", 0, Number.POSITIVE_INFINITY, e), this._maxUploadRetryTime = e
    }
    ...
    set maxOperationRetryTime(e) {
        validateNumber("time", 0, Number.POSITIVE_INFINITY, e), this._maxOperationRetryTime = e
    }
    ...
    _makeRequest(e, t, r, n, o = !0) {
        if (this._deleted) return new FailRequest(appDeleted());
        {
            const s = function makeRequest(e, t, r, n, o, s, i = !0) {
                const a = makeQueryString(e.urlParams), l = e.url + a, c = Object.assign({}, e.headers);
                return function addGmpidHeader_(e, t) {
                    t && (e["X-Firebase-GMPID"] = t)
                }(c, t), function addAuthHeader_(e, t) {
                    null !== t && t.length > 0 && (e.Authorization = "Firebase " + t)
                }(c, r), function addVersionHeader_(e, t) {
                    e["X-Firebase-Storage-Version"] = "webjs/" + (null != t ? t : "AppManager")
                }(c, s), function addAppCheckHeader_(e, t) {
                    null !== t && (e["X-Firebase-AppCheck"] = t)
                }(c, n), new NetworkRequest(l, e.method, c, e.body, e.successCodes, e.additionalRetryCodes, e.handler, e.errorHandler, e.timeout, e.progressCallback, o, i)
    ...
    async makeRequestWithTokens(e, t) {
        const [r, n] = await Promise.all([this._getAuthToken(), this._getAppCheckToken()]);
...
function getBytes(e, t) {
    return function getBytesInternal(e, t) {
        ...
        return e.storage.makeRequestWithTokens(r, newBytesConnection).then((e => void 0 !== t ? e.slice(0, t) : e))
...
function uploadString(e, t, r, n) {
    return function uploadString$1(e, t, r = l.RAW, n) {
        ...
        const o = dataFromString(r, t), s = Object.assign({}, n);
        return null == s.contentType && null != o.contentType && (s.contentType = o.contentType), uploadBytes$1(e, o.data, s)
    }(e = getModularInstance(e), t, r, n)
}

function uploadBytesResumable(e, t, r) {
    return function uploadBytesResumable$1(e, t, r) {
        return e._throwIfRoot("uploadBytesResumable"), new UploadTask(e, new FbsBlob(t), r)
...
function getBlob(e, t) {
    return function getBlobInternal(e, t) {
        ...
        const r = getBytes$1(e.storage, e._location, t);
        return e.storage.makeRequestWithTokens(r, newBlobConnection).then((e => void 0 !== t ? e.slice(0, t) : e))
    }(e = getModularInstance(e), t)
}
...
function factory(e, {instanceIdentifier: t}) {
    const r = e.getProvider("app").getImmediate(), o = e.getProvider("auth-internal"),
        s = e.getProvider("app-check-internal");
    return new FirebaseStorageImpl(r, o, s, t, n)
}

!function registerStorage() {
    t(new Component("storage", factory, "PUBLIC").setMultipleInstances(!0)), r(d, "0.12.0", ""), r(d, "0.12.0", "esm2017")
}();
export {
    StorageError,
